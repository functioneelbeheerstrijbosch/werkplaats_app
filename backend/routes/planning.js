...
router.get('/monteurs', async (req, res) => {
  try {
    ...
    // wachtwoord_hash/nfc_token_hash nooit meesturen — zelfde reden als bij
    // POST /auth/login resp. /auth/nfc-login.
    res.json(rows.map(({ wachtwoord_hash, nfc_token_hash, ...r }) => r));
  } catch (err) {
